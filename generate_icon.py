#!/usr/bin/env python3
"""Generate Rain Walker's mod menu icon: rain falling onto vanilla ice.
...
OUT = os.path.join(HERE, "src/main/resources/assets/rain-walker/icon.png")
...
RAIN = (0xF4, 0xF9, 0xFF, 0xFF)
DROP = (0x3E, 0x6C, 0xC0, 0xFF)
COLUMNS = (1, 4, 7, 10, 13)
TOPS = (0, 5, 2, 6, 3)   # fixed, so the icon is byte for byte reproducible


def build_icon():
    """Rain falling onto ice: the enchantment freezes Blocks.ICE underfoot, so
    vanilla ice is the ground and the streaks are what puts you on it."""
    ice = [[shade(px, 0.88) for px in row] for row in vanilla("block/ice.png")]
    for column, top in zip(COLUMNS, TOPS):
        for y in range(top, min(15, top + 4)):
            ice[y][column] = RAIN
        ice[min(15, top + 4)][column] = DROP
    return scale(ice, 8)
